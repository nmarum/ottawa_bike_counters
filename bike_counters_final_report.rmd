...
This report documents the exploratory analysis and machine learning model building process for a HarvardX Data Science Professional Certificate capstone project.  It is uses publicly available City of Ottawa bike ride counter data from public pathways in Ottawa, CANADA as well as publicly available weather data from Environment Canada.
...
```
Comparing the amount of recorded snow on the ground and the number of rides shows a clear trend that _any_ recorded snow on the ground reduces the number of rides 
significantly, though there are still a handful of hardy Ottawa bike riders that seem to ride their bikes year round along the Colonel By Drive pathway no matter how much snow is on the ground.
